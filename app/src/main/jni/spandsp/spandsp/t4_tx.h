...
SPAN_DECLARE(t4_state_t * ) t4_tx_init(t4_state_t * s , const char *file,
int start_page,
int stop_page ) ;
...
SPAN_DECLARE(int)
t4_tx_start_page(t4_state_t * s ) ;

/*! \brief Prepare the current page for a resend.
    \param s The T.4 context.
    \return zero for success, -1 for failure. */
SPAN_DECLARE(int)
t4_tx_restart_page(t4_state_t * s ) ;
...
SPAN_DECLARE(int)
t4_tx_next_page_has_different_format(t4_state_t * s ) ;

/*! \brief Complete the sending of a page.
    \param s The T.4 context.
    \return zero for success, -1 for failure. */
SPAN_DECLARE(int)
t4_tx_end_page(t4_state_t * s ) ;
...
SPAN_DECLARE(int)
t4_tx_get_chunk(t4_state_t * s , uint8_t buf[],
int max_len ) ;
...
/*! \brief End the transmission of a document. Tidy up, close the file and
    ...
    \return 0 for success, otherwise -1. */
SPAN_DECLARE(int)
t4_tx_free(t4_state_t * s ) ;

/*! \brief Set the encoding for the encoded data.
    \param s The T.4 context.
    \param encoding The encoding. */
SPAN_DECLARE(void)
t4_tx_set_tx_encoding(t4_state_t * s , int encoding ) ;
...
SPAN_DECLARE(void)
t4_tx_set_header_info(t4_state_t * s , const char *info ) ;
...
/*! \brief Set the row read handler for a T.4 transmit context.
    ...
    \return 0 for success, otherwise -1. */
SPAN_DECLARE(int)
t4_tx_set_row_read_handler(t4_state_t * s , t4_row_read_handler_t handler,
void *user_data ) ;
