...
SPAN_DECLARE(int)
t30_set_tx_nsf(t30_state_t * s , const uint8_t *nsf,
int len ) ;
...
SPAN_DECLARE(size_t)
t30_get_tx_nsf(t30_state_t * s , const uint8_t *nsf[] ) ;
...
/*! Get an NSF frame to be associated with a T.30 context.
    ...
    \return the length of the NSF message. */
SPAN_DECLARE(size_t)
t30_get_rx_nsf(t30_state_t * s , const uint8_t *nsf[] ) ;
...
/*! Set the transmitted NSC frame to be associated with a T.30 context.
    ...
    \return 0 for OK, else -1. */
SPAN_DECLARE(int)
t30_set_tx_nsc(t30_state_t * s , const uint8_t *nsc,
int len ) ;
...
SPAN_DECLARE(size_t)
t30_get_tx_nsc(t30_state_t * s , const uint8_t *nsc[] ) ;
...
/*! Get an NSC frame to be associated with a T.30 context.
    ...
    \return the length of the NSC message. */
SPAN_DECLARE(size_t)
t30_get_rx_nsc(t30_state_t * s , const uint8_t *nsc[] ) ;
...
/*! Set the transmitted NSS frame to be associated with a T.30 context.
    ...
    \return 0 for OK, else -1. */
SPAN_DECLARE(int)
t30_set_tx_nss(t30_state_t * s , const uint8_t *nss,
int len ) ;
...
SPAN_DECLARE(size_t)
t30_get_tx_nss(t30_state_t * s , const uint8_t *nss[] ) ;
...
/*! Get an NSS frame to be associated with a T.30 context.
    ...
    \return the length of the NSS message. */
SPAN_DECLARE(size_t)
t30_get_rx_nss(t30_state_t * s , const uint8_t *nss[] ) ;
...
/*! Set the transmitted identifier associated with a T.30 context.
    ...
    \return 0 for OK, else -1. */
SPAN_DECLARE(int)
t30_set_tx_ident(t30_state_t * s , const char *id ) ;
...
t30_get_tx_ident(t30_state_t * s ) ;
...
/*! Get the transmitted identifier associated with a T.30 context.
    ...
    \return A pointer to the identifier. */
SPAN_DECLARE(const char *)
t30_get_rx_ident(t30_state_t * s ) ;
...
/*! Set the transmitted sub-address associated with a T.30 context.
    ...
    \return 0 for OK, else -1. */
SPAN_DECLARE(int)
t30_set_tx_sub_address(t30_state_t * s , const char *sub_address ) ;
...
SPAN_DECLARE(const char *)
t30_get_tx_sub_address(t30_state_t * s ) ;
...
/*! Get the received sub-address associated with a T.30 context.
    ...
    \return A pointer to the sub-address. */
SPAN_DECLARE(const char *)
t30_get_rx_sub_address(t30_state_t * s ) ;
...
/*! Set the transmitted selective polling address (i.e. the one we will send to the far
    ...
    \return 0 for OK, else -1. */
SPAN_DECLARE(int)
t30_set_tx_selective_polling_address(t30_state_t * s , const char *selective_polling_address ) ;
...
SPAN_DECLARE(const char *)
t30_get_tx_selective_polling_address(t30_state_t * s ) ;
...
/*! Get the received selective polling address (i.e. the one we will send to the far
    ...
    \return A pointer to the selective polling address. */
SPAN_DECLARE(const char *)
t30_get_rx_selective_polling_address(t30_state_t * s ) ;
...
/*! Set the transmitted polled sub-address (i.e. the one we will send to the far
    ...
    \return 0 for OK, else -1. */
SPAN_DECLARE(int)
t30_set_tx_polled_sub_address(t30_state_t * s , const char *polled_sub_address ) ;
...
SPAN_DECLARE(const char *)
t30_get_tx_polled_sub_address(t30_state_t * s ) ;
...
/*! Get the received polled sub-address (i.e. the one we will send to the far
    ...
    \return A pointer to the polled sub-address. */
SPAN_DECLARE(const char *)
t30_get_rx_polled_sub_address(t30_state_t * s ) ;
...
/*! Set the transmitted sender ident (i.e. the one we will send to the far
    ...
    \return 0 for OK, else -1. */
SPAN_DECLARE(int)
t30_set_tx_sender_ident(t30_state_t * s , const char *sender_ident ) ;
...
SPAN_DECLARE(const char *)
t30_get_tx_sender_ident(t30_state_t * s ) ;
...
/*! Get the received sender ident (i.e. the one we will send to the far
    ...
    \return A pointer to the sender ident. */
SPAN_DECLARE(const char *)
t30_get_rx_sender_ident(t30_state_t * s ) ;
...
/*! Set the transmitted password (i.e. the one we will send to the far
    ...
    \return 0 for OK, else -1. */
SPAN_DECLARE(int)
t30_set_tx_password(t30_state_t * s , const char *password ) ;
...
SPAN_DECLARE(const char *)
t30_get_tx_password(t30_state_t * s ) ;
...
/*! Get the received password (i.e. the one we will send to the far
    ...
    \return A pointer to the password. */
SPAN_DECLARE(const char *)
t30_get_rx_password(t30_state_t * s ) ;
...
/*! Set the transmitted ??? (i.e. the one we will send to the far
    ...
    \return 0 for OK, else -1. */
SPAN_DECLARE(int)
t30_set_tx_tsa(t30_state_t * s , int type,
const char *address,
int len ) ;
...
SPAN_DECLARE(size_t)
t30_get_tx_tsa(t30_state_t * s , int *type,
const char *address[] ) ;
...
/*! Get the received ??? (i.e. the one we will send to the far
    ...
    \return The length of the address. */
SPAN_DECLARE(size_t)
t30_get_rx_tsa(t30_state_t * s , int *type,
const char *address[] ) ;
...
/*! Set the transmitted ??? (i.e. the one we will send to the far
    ...
    \return 0 for OK, else -1. */
SPAN_DECLARE(int)
t30_set_tx_ira(t30_state_t * s , int type,
const char *address,
int len ) ;
...
/*! Get the received ??? (i.e. the one we will send to the far
    ...
    \return The length of the address. */
SPAN_DECLARE(size_t)
t30_get_tx_ira(t30_state_t * s , int *type,
const char *address[] ) ;
...
/*! Get the received ??? (i.e. the one we will send to the far
    ...
    \return The length of the address. */
SPAN_DECLARE(size_t)
t30_get_rx_ira(t30_state_t * s , int *type,
const char *address[] ) ;
...
/*! Set the transmitted ??? (i.e. the one we will send to the far
    ...
    \return 0 for OK, else -1. */
SPAN_DECLARE(int)
t30_set_tx_cia(t30_state_t * s , int type,
const char *address,
int len ) ;
...
/*! Get the received ??? (i.e. the one we will send to the far
    ...
    \return The length of the address. */
SPAN_DECLARE(size_t)
t30_get_tx_cia(t30_state_t * s , int *type,
const char *address[] ) ;
...
/*! Get the received ??? (i.e. the one we will send to the far
    ...
    \return 0 for OK, else -1. */
SPAN_DECLARE(size_t)
t30_get_rx_cia(t30_state_t * s , int *type,
const char *address[] ) ;
...
/*! Set the transmitted ??? (i.e. the one we will send to the far
    ...
    \return 0 for OK, else -1. */
SPAN_DECLARE(int)
t30_set_tx_isp(t30_state_t * s , int type,
const char *address,
int len ) ;
...
/*! Get the received ??? (i.e. the one we will send to the far
    ...
    \return 0 for OK, else -1. */
SPAN_DECLARE(size_t)
t30_get_tx_isp(t30_state_t * s , int *type,
const char *address[] ) ;
...
/*! Get the received ??? (i.e. the one we will send to the far
    ...
    \return 0 for OK, else -1. */
SPAN_DECLARE(size_t)
t30_get_rx_isp(t30_state_t * s , int *type,
const char *address[] ) ;
...
/*! Set the transmitted ??? (i.e. the one we will send to the far
    ...
    \return 0 for OK, else -1. */
SPAN_DECLARE(int)
t30_set_tx_csa(t30_state_t * s , int type,
const char *address,
int len ) ;
...
/*! Get the received ??? (i.e. the one we will send to the far
    ...
    \return The length of the address. */
SPAN_DECLARE(size_t)
t30_get_tx_csa(t30_state_t * s , int *type,
const char *address[] ) ;
...
/*! Get the received ??? (i.e. the one we will send to the far
    ...
    \return 0 for OK, else -1. */
SPAN_DECLARE(size_t)
t30_get_rx_csa(t30_state_t * s , int *type,
const char *address[] ) ;
...
/*! Set the transmitted header information associated with a T.30 context.
    ...
    \return 0 for OK, else -1. */
SPAN_DECLARE(int)
t30_set_tx_page_header_info(t30_state_t * s , const char *info ) ;
...
SPAN_DECLARE(size_t)
t30_get_tx_page_header_info(t30_state_t * s , char *info ) ;
...
SPAN_DECLARE(const char *)
t30_get_rx_country(t30_state_t * s ) ;
...
SPAN_DECLARE(const char *)
t30_get_rx_vendor(t30_state_t * s ) ;
...
SPAN_DECLARE(const char *)
t30_get_rx_model(t30_state_t * s ) ;
...
SPAN_DECLARE(void)
t30_set_rx_file(t30_state_t * s , const char *file,
int stop_page ) ;
...
SPAN_DECLARE(void)
t30_set_tx_file(t30_state_t * s , const char *file,
int start_page,
int stop_page ) ;
...
SPAN_DECLARE(void)
t30_set_iaf_mode(t30_state_t * s , int iaf ) ;
...
SPAN_DECLARE(int)
t30_set_ecm_capability(t30_state_t * s , int enabled ) ;
...
/*! Specify the output encoding for TIFF files created during FAX reception.
    ...
    \return 0 if OK, else -1. */
SPAN_DECLARE(int)
t30_set_rx_encoding(t30_state_t * s , int encoding ) ;
...
/*! Specify the minimum scan line time supported by a T.30 context.
    ...
    \return 0 if OK, else -1. */
SPAN_DECLARE(int)
t30_set_minimum_scan_line_time(t30_state_t * s , int min_time ) ;
...
/*! Specify which modem types are supported by a T.30 context.
    ...
    \return 0 if OK, else -1. */
SPAN_DECLARE(int)
t30_set_supported_modems(t30_state_t * s , int supported_modems ) ;
...
/*! Specify which compression types are supported by a T.30 context.
    ...
    \return 0 if OK, else -1. */
SPAN_DECLARE(int)
t30_set_supported_compressions(t30_state_t * s , int supported_compressions ) ;
...
/*! Specify which resolutions are supported by a T.30 context.
    ...
    \return 0 if OK, else -1. */
SPAN_DECLARE(int)
t30_set_supported_resolutions(t30_state_t * s , int supported_resolutions ) ;
...
/*! Specify which images sizes are supported by a T.30 context.
    ...
    \return 0 if OK, else -1. */
SPAN_DECLARE(int)
t30_set_supported_image_sizes(t30_state_t * s , int supported_image_sizes ) ;
...
/*! Specify which special T.30 features are supported by a T.30 context.
    ...
    \return 0 if OK, else -1. */
SPAN_DECLARE(int)
t30_set_supported_t30_features(t30_state_t * s , int supported_t30_features ) ;
...
SPAN_DECLARE(void)
t30_set_status(t30_state_t * s , int status ) ;
...
/*! Specify a period of responding with receiver not ready.
    ...
    \return 0 if OK, else -1. */
SPAN_DECLARE(int)
t30_set_receiver_not_ready(t30_state_t * s , int count ) ;
...
SPAN_DECLARE(void)
t30_set_phase_b_handler(t30_state_t * s , t30_phase_b_handler_t *handler,
void *user_data ) ;
...
/*! Set a callback function for T.30 phase D handling.
    ...
    \param user_data An opaque pointer passed to the callback function. */
SPAN_DECLARE(void)
t30_set_phase_d_handler(t30_state_t * s , t30_phase_d_handler_t *handler,
void *user_data ) ;
...
/*! Set a callback function for T.30 phase E handling.
    ...
    \param user_data An opaque pointer passed to the callback function. */
SPAN_DECLARE(void)
t30_set_phase_e_handler(t30_state_t * s , t30_phase_e_handler_t *handler,
void *user_data ) ;
...
/*! Set a callback function for T.30 end of document handling.
    ...
    \param user_data An opaque pointer passed to the callback function. */
SPAN_DECLARE(void)
t30_set_document_handler(t30_state_t * s , t30_document_handler_t *handler,
void *user_data ) ;
...
/*! Set a callback function for T.30 frame exchange monitoring. This is called from the heart
    ...
    \param user_data An opaque pointer passed to the callback function. */
SPAN_DECLARE(void)
t30_set_real_time_frame_handler(t30_state_t * s , t30_real_time_frame_handler_t *handler,
void *user_data ) ;
