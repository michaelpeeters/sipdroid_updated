...
static __inline__ uint32_t least_significant_one32(uint32_t
x )
{
return ( x & ( - ( int32_t ) x )) ;
}
...
/*! \brief Find the most significant one in a word, and return a word
    ...
    \param x The word to be searched.
    \return The word with the single set bit. */
static __inline__ uint32_t most_significant_one32(uint32_t
x)
{
#if defined(__i386__) || defined(__x86_64__) || defined(__ppc__) || defined(__powerpc__)
return 1 << top_bit(x);
...
static __inline__ int parity8(uint8_t
x)
{
x = (x ^ (x >> 4)) & 0x0F;
return (0x6996 >> x) & 1;
...
/*! \brief Find the parity of a 16 bit word.
    ...
    \return 1 for odd, or 0 for even. */
static __inline__ int parity16(uint16_t
x)
{
x ^= (x >> 8);
x = (x ^ (x >> 4)) & 0x0F;
...
/*! \brief Find the parity of a 32 bit word.
    \param x The word to be checked.
    \return 1 for odd, or 0 for even. */
static __inline__ int parity32(uint32_t
x)
{
x ^= (x >> 16);
x ^= (x >> 8);
