...
SPAN_DECLARE(t4_state_t * ) t4_rx_init(t4_state_t * s , const char *file,
int output_encoding ) ;
...
SPAN_DECLARE(int)
t4_rx_put_bit(t4_state_t * s , int bit ) ;
...
SPAN_DECLARE(int)
t4_rx_put_byte(t4_state_t * s , uint8_t byte ) ;
...
/*! \brief Put a byte of the current document page.
    ...
    \return TRUE when the byte ends the document page, otherwise FALSE. */
SPAN_DECLARE(int)
t4_rx_put_chunk(t4_state_t * s , const uint8_t buf[],
int len ) ;
...
/*! \brief Set the row write handler for a T.4 receive context.
    ...
    \return 0 for success, otherwise -1. */
SPAN_DECLARE(int)
t4_rx_set_row_write_handler(t4_state_t * s , t4_row_write_handler_t handler,
void *user_data ) ;
...
SPAN_DECLARE(void)
t4_get_transfer_statistics(t4_state_t * s , t4_stats_t *t ) ;
...
t4_encoding_to_str(int encoding);
