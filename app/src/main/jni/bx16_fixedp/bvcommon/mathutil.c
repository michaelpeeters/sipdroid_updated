...
void Log2(
    ...
    if (x <= 0) {
        ...
        *frac_comp = 0;
        ...
        a0 = L_shr(L_sub(a0, (Word32) 0x40000000),
                   8);      // Q16 index into table - note zero'ing of leading 1
...
Word16 sqrts(Word16 x) {
    ...
    if (x <= 0) {
        ...
        a0 = L_mac(a0, sub_frac,
                   sub_tab);           // Q31 linear interpolation between table entries
        if (exp & 0x0001) {
...
void sqrt_i(Word16 x_man, Word16 x_exp, Word16 *y_man, Word16 *y_exp) {
    ...
    if (x_man <= 0) {
        ...
        *y_exp = 0;
        ...
        x_expb = add(x_exp,
                     exp);                    // update exponent for x (left-shifting by additionl exp)
        x_expb = sub(x_expb,
                     15);                    // we need to take sqrt of 0-32767 number but table is for 0-1
        ...
        if (x_expb & 0x0001) {
            if (y <
                0x5A82) {                           // 1*sqrt(2) in Q14) - with div_s(y1, y2), y2 must be >= y1
            ...
            *y_exp = sub(29,
                         exp);                    // 0x4000 is 1 in Q14 and div_s returns fraction divide in Q15
        }
    }
